...
function hexToVec3(color: string): Vec3 {
  ...
  if (rgbMatch) {
    return new Vec3(
      Number.parseInt(rgbMatch[1]) / 255,
      Number.parseInt(rgbMatch[2]) / 255,
      Number.parseInt(rgbMatch[3]) / 255,
    )
  }
...
void main() {
  ...
  useEffect(() => {
    ...
    const update = (t: number) => {
      ...
      program.uniforms.hover.value += (effectiveHover - currentHover) * 0.1
  ...
  return <div ref={ctnDom} className={`absolute inset-0 ${className}`} />
}
